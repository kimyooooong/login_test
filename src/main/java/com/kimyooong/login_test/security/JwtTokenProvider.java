...
public class JwtTokenProvider {
    ...
    // JWT 토큰 생성
    public String createToken(String phoneNumber, List<String> roles) {
        Claims claims = Jwts.claims().setSubject(phoneNumber); // JWT payload 에 저장되는 정보단위, 보통 여기서 user를 식별하는 값을 넣는다.
        claims.put("roles", roles); // 정보는 key / value 쌍으로 저장된다.
        Date now = new Date();

        log.info("data : {}", claims);
        return Jwts.builder()
